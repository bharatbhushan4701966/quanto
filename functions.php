...
add_shortcode('cmr_similar_reports', function() {
    ...
    if ( false === $cached_html || ( is_user_logged_in() && isset($_GET['refresh_cache']) ) ) {
        ...
        // Extract the inline CSS tags from Elementor (external links)
        $inline_css = '';
        if ( preg_match_all( '/elementor\/css\/post-(\d+)\.css/i', $body, $id_matches ) ) {
            $post_ids = array_unique( $id_matches[1] );
            foreach ( $post_ids as $pid ) {
        ...
        // Also extract ANY inline style tags generated by Elementor (if external CSS is disabled)
        ...
        // Extract by finding the opening tag and doing a greedy match until the end of main/article or a known footer
        if ( preg_match( '/(<div[^>]*data-elementor-type="[^"]+"[^>]*data-elementor-id="' . $post_id . '"[^>]*>.*?)(?:<\/main>|<\/article>|<footer|<!-- \.elementor -->|<\/body>)/is', $body, $matches ) ) {
            $content_html = $matches[1];
            // Since we matched until </main> or </footer>, we might have unclosed divs if we just take $matches[1] blindly.
            // A safer approach if we just grab everything from the start of the elementor div
        }
        // A safer way: just use the original method but manually inject the extracted CSS!
        // We have the inline CSS now.
        // Let's render the content normally using Elementor, and PREPEND the inline CSS.
        if ( class_exists( '\Elementor\Plugin' ) ) {
            $content_html = \Elementor\Plugin::instance()->frontend->get_builder_content_for_display( $post_id, true );
        } else {
            $content_html = '<!-- Elementor plugin not active -->';
        }
